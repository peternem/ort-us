...
//echo "<b>Current User Role</b>: ".get_user_role();
	...
	<div class="col-<?php echo ( ( $col = $col * -1 ) < 0 ) ? 1 : 2; ?> address">
		<header class="title">
			<h3><?php echo $title; ?></h3>
                        <?php 
                        $xyz = wc_get_endpoint_url( 'edit-address', $name ); 
                        $id = substr($xyz, strrpos($xyz, '/') + 1);
                        ?>
                        <?php 
                        // Check for user roles. See config-roles.php
                        if ($GLOBALS['role_flag'] === 0) { ?>
                                <a id="<?php echo $id; ?>" href="<?php echo wc_get_endpoint_url( 'edit-address', $name ); ?>" class="edit"><?php _e( 'Edit', 'woocommerce' ); ?></a>
                        <?php } ?>
                        <?php if ($GLOBALS['role_flag'] === 1 && $id !== "shipping") { ?>
                                <a id="<?php echo $id; ?>" href="<?php echo wc_get_endpoint_url( 'edit-address', $name ); ?>" class="edit"><?php _e( 'Edit', 'woocommerce' ); ?></a>
                        <?php } ?>
                </header>
		<address>
